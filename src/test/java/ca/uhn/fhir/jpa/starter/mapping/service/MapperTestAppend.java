...
public class MapperTestAppend {
	...
	private final String hl7v2Message =
		"MSH|^~\\&|SendingApp|SendingFac|ReceivingApp|ReceivingFac|20120411070545||ORU^R01|59689|P|2.3\r"
			+ "PID|1|PID21^PID22^PID23^Dx Care&1.2.250.1.38.3.1.101&ISO^PT|PID31^PID32^PID33^MIE&1.2.840.114398.1.100&ISO^MR|PID41^PID22^PID43^PID441&1.2.3.4.5&HCD^SS|MOUSE^MINNIE^S||19240101|ALIASMOUSE^ALIASMINNIE||123 MOUSEHOLE LN^PID112^FORT WAYNE^IN^46808^USA^H^^ALLEN^001234||(260)555-1234^PRN^PH^minnie.mouse@example.org^1^260^5551234^99^Home phone|(260)555-9876^WPN^PH^minnie.work@example.org^1^260^5559876^123^Work phone|||||999-88-7777|D123456789^1.2.250.1.38.3.1.102^20300101||||||||||||\r"
			+ "PV1|1|O|||||71^DUCK^DONALD||||||||||||12376|||||||||||||||||||||||||20120410160227||||||\r"
			+ "ORC|RE||12376|||||||100^DUCK^DASIY||71^DUCK^DONALD|^^^||20120411070545|||||\r"
			+ "OBR|1|PLACER12376^OE^1.2.3.4.5^ISO|FILLER98765^LAB^9.8.7.6.5^HL7|cbc^CBC^R|R|198703281530|20120410160227|||22^GOOF^GOOFY^^^^MD^L|||Fasting: No|20120410162500|BLD|71^DUCK^DONALD^^^^DR.|(555)555-1234|||||||20120410163000|||||^^^^^R|||||||||||||||||||85025|\r"
			+ "OBX|1|SN|wbc^Wbc^Local^6690-2^Wbc^LN|1|<>^12^3^5|/nl|3.8-11.0|N|0.95|A||20120410|SECRET|20120410160227|lab|12^XYZ LAB|\r";
	...
	void append_shouldConvertNbspToSpace() throws Exception {
		...
		target.addParameter().setValue(new StringType("DOE"));
		target.addParameter().setValue(new StringType("nbsp"));
		target.addParameter().setValue(new StringType("JOHN"));

		MappingContext ctx = mock(MappingContext.class);
		when(ctx.getTarget()).thenReturn(target);
		when(ctx.getVariables()).thenReturn(mock(Variables.class));

		Method m = Mapper.class.getDeclaredMethod(
			"runTransform",
